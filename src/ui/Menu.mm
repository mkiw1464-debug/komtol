...
static void Save(){
    ...
    [ud setBool:g_Aimbot.enabled forKey:@"a_on"];
    [ud setBool:g_Aimbot.silent  forKey:@"a_si"];
    [ud setBool:g_Aimbot.showFov forKey:@"a_fv"];
    [ud setFloat:g_Aimbot.fov    forKey:@"a_fr"];
    [ud setInteger:g_Aimbot.targetBone forKey:@"a_bn"];
    [ud setBool:g_ESP.enabled  forKey:@"e_on"];
    [ud setBool:g_ESP.showName forKey:@"e_nm"];
    [ud setBool:g_ESP.showBox  forKey:@"e_bx"];
    [ud setBool:g_ESP.showLine forKey:@"e_ln"];
    [ud setBool:g_ESP.showHP   forKey:@"e_hp"];
    [ud setBool:g_SP forKey:@"s_sp"];
    [ud synchronize];
}
static void Load(){
    NSUserDefaults *ud=[NSUserDefaults standardUserDefaults];
    if(![ud objectForKey:@"a_on"])return;
    g_Aimbot.enabled=(bool)[ud boolForKey:@"a_on"];
    g_Aimbot.silent =(bool)[ud boolForKey:@"a_si"];
    g_Aimbot.showFov=(bool)[ud boolForKey:@"a_fv"];
    g_Aimbot.fov    =[ud floatForKey:@"a_fr"];
    g_Aimbot.targetBone=(int)[ud integerForKey:@"a_bn"];
    g_ESP.enabled =(bool)[ud boolForKey:@"e_on"];
    g_ESP.showName=(bool)[ud boolForKey:@"e_nm"];
    g_ESP.showBox =(bool)[ud boolForKey:@"e_bx"];
    g_ESP.showLine=(bool)[ud boolForKey:@"e_ln"];
    g_ESP.showHP  =(bool)[ud boolForKey:@"e_hp"];
    g_SP=(bool)[ud boolForKey:@"s_sp"];
}
...
@implementation FFH
+(instancetype)sh{static FFH*h;static dispatch_once_t t;dispatch_once(&t,^{h=[FFH new]});return h;}
-(void)tap{ToggleMenu();}
-(void)close{ToggleMenu();}
...
-(void)segBone:(UISegmentedControl*)s{int m[]={0,1,6,19};int i=(int)s.selectedSegmentIndex;g_Aimbot.targetBone=(i>=0&&i<4)?m[i]:0;Save();}
...
// ─── Page builders ────────────────────────────────────────
#define T [UIColor colorWithRed:.45f green:.45f blue:1 alpha:1]
...
static UIView *PageSettings(CGFloat mw,CGFloat ch){
    ...
    UILabel *ver=[[UILabel alloc]initWithFrame:CGRectMake(14,ch-22,mw-28,18)];
...
static void BuildPanel(UIView *root){
    ...
    CGFloat mw=296,mh=390,mx=(ss.width-mw)*.5f,my=(ss.height-mh)*.5f;
    g_Panel=[[UIView alloc]initWithFrame:CGRectMake(mx,my,mw,mh)];
    g_Panel.layer.cornerRadius=16;g_Panel.clipsToBounds=YES;
    g_Panel.layer.borderColor=[UIColor colorWithWhite:1 alpha:.12].CGColor;
    ...
    tl.text=@"FFNET  ·  iOS V1.0.0";tl.textColor=W;tl.font=[UIFont boldSystemFontOfSize:13];
    [bar addSubview:tl];
...
static void BuildFloatButton(UIView *root){
    CGSize ss=UIScreen.mainScreen.bounds.size;
    CGFloat bsz=50;
    FFBtn *btn=[[FFBtn alloc]initWithFrame:CGRectMake(ss.width-bsz-10,ss.height*.32f,bsz,bsz)];
    btn.layer.cornerRadius=bsz*.5f;btn.clipsToBounds=YES;
    btn.layer.borderColor=[UIColor colorWithWhite:1 alpha:.25].CGColor;btn.layer.borderWidth=1;
    // Blur bg
    UIBlurEffect *be=[UIBlurEffect effectWithStyle:UIBlurEffectStyleSystemUltraThinMaterialDark];
    UIVisualEffectView *bv=[[UIVisualEffectView alloc]initWithEffect:be];
    ...
    logo.text=@"⚡";logo.textAlignment=NSTextAlignmentCenter;
    logo.font=[UIFont systemFontOfSize:22];logo.userInteractionEnabled=NO;[btn addSubview:logo];
    [btn addTarget:[FFH sh] action:@selector(tap) forControlEvents:UIControlEventTouchUpInside];
    g_FloatBtn=btn;[root addSubview:btn];
}

// ─── Overlay window ───────────────────────────────────────
static UIWindow *MakeWindow(CGFloat level){
    UIWindow *w=nil;
    if(@available(iOS 13.0,*)){
        for(UIScene *s in UIApplication.sharedApplication.connectedScenes){
            if(![s isKindOfClass:[UIWindowScene class]])continue;
            if(s.activationState==UISceneActivationStateForegroundActive){
                w=[[UIWindow alloc]initWithWindowScene:(UIWindowScene*)s];break;
            }
        }
    }
    if(!w) w=[[UIWindow alloc]initWithFrame:UIScreen.mainScreen.bounds];
    w.windowLevel=level;w.backgroundColor=UIColor.clearColor;
    w.rootViewController=[UIViewController new];return w;
}

void ToggleMenu(){
    if(![NSThread isMainThread]){dispatch_async(dispatch_get_main_queue(),^{ToggleMenu();});return;}
    g_Visible=!g_Visible;
    g_Panel.hidden=!g_Visible;
    [UIView animateWithDuration:.15 animations:^{g_FloatBtn.alpha=g_Visible?.3f:1.f;}];
    if(g_Visible&&g_SP){
...
void InitMenu(){
    ...
    Load();
    g_Win=MakeWindow(UIWindowLevelAlert+80);
    g_Win.hidden=NO;[g_Win makeKeyAndVisible];
    ...
    NSLog(@"[FFNET] Menu ready — tap ⚡ button");
}
