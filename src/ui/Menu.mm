...
// ─── Safe key window — iOS 13+ UIWindowScene, no deprecated API ──────────────
static UIWindow *GetKeyWindow() {
    ...
    if (@available(iOS 13.0, *)) {
        for (UIScene *scene in UIApplication.sharedApplication.connectedScenes) {
            ...
            // No keyWindow found in active scene — return first window
...
- (void)handleTap:(UITapGestureRecognizer*)gr {
    if (gr.state == UIGestureRecognizerStateEnded) HandleTap();
}
@end
...
static void BuildMenu() {
    ...
    // iOS 13+ — create window attached to active window scene
    if (@available(iOS 13.0, *)) {
...
// ─── Toggle menu — NSAssert replaced with runtime check (works in C functions)
void ToggleMenu() {
    ...
    if (menuVisible) {
        ...
        menuWindow.hidden = YES;
        // Return key status to FF window
        NSArray<UIWindow*> *all = GetAllWindows();
        for (UIWindow *w in all) {
            if (w != menuWindow) { [w makeKeyWindow]; break; }
        }
    }
}
...
// ─── Init — NSAssert replaced, safe in C function context ─────────────────────
void InitMenu() {
    if (![NSThread isMainThread]) {
    ...
    // ── Trigger 2: Volume Up button (main trigger, works in-game) ────────────
    // Hook MPVolumeView / AVAudioSession volume change notification
    [[NSNotificationCenter defaultCenter]
        addObserverForName:@"AVSystemController_SystemVolumeDidChangeNotification"
                    object:nil
                     queue:[NSOperationQueue mainQueue]
                usingBlock:^(NSNotification *note) {
        NSString *category = note.userInfo[@"AVSystemController_AudioCategoryNotificationParameter"];
        NSString *reason   = note.userInfo[@"AVSystemController_AudioVolumeChangeReasonNotificationParameter"];
        // Only trigger on explicit button press, not system/app volume changes
        if ([category isEqualToString:@"Audio/Video"] &&
            [reason isEqualToString:@"ExplicitVolumeChange"]) {
            HandleTap(); // reuse tap counter — press 3x to open
        }
    }];

    // Keep system volume from actually changing (prevent HUD popup)
    dispatch_async(dispatch_get_main_queue(), ^{
        // Embed hidden MPVolumeView to suppress iOS volume HUD
        Class mpClass = NSClassFromString(@"MPVolumeView");
        if (mpClass) {
            UIView *vv = [[mpClass alloc] initWithFrame:CGRectMake(-500,-500,1,1)];
            [kw addSubview:vv];
        }
    });

    // ESP overlay — sits on top of FF, under menu
    ...
    NSLog(@"[FFNET] Menu + DrawView ready");
}

// ─── Tap handler category ─────────────────────────────────────────────────────
